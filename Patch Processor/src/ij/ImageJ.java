...
@revisor Baihan Lin (SunnyLin@uw.edu)
*/
public class ImageJ extends Frame implements ActionListener, 
	...
	public static void doImageJMain(String args[]) {
	...
	*/
	...
	private void outputJComp() {
		Map<String, JComponent[]> Comps = WindowManager.getAllComponents();                 // My Changes!!!
		if (Comps!=null) {
			for (String window : Comps.keySet()) {
				if (Comps.get(window) !=null) {
					for (int i = 0; i < Comps.get(window).length; i++) {
						pp.VisualPatchProcessorMain.outCanvas.add(Comps.get(window)[i]);
					}
//					for (Component c : WindowManager.getNonImageWindows()) {
//						pp.VisualPatchProcessorMain.outCanvas.paintAll(c);;
//					}
