...
public class WindowManager {
	...
	public static Map<String, JComponent[]> getAllComponents() {
		Map<String, JComponent[]> comps = new HashMap<String, JComponent[]>();
		for (int i=0; i<nonImageList.size(); i++) {
			Object win = nonImageList.elementAt(i);
			if (win instanceof Frame) {
				comps.put(((Frame) win).getName(), (JComponent[]) ((Frame) win).getComponents());
			}
		}
		for (int i=0; i<imageList.size(); i++) {
